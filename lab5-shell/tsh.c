...
{
    // 解析命令
    char tokens[MAXARGS][64]; // 不知道单个参数64是否正确
    char *argv[MAXARGS];
    for (int i = 0; i < MAXARGS; i++)
    {
        argv[i] = tokens[i];
    }
    int is_bg = parseline(cmdline, argv); // 是否为后台程序
    if (argv[0] == NULL)
    {
        return;
    }
    ...
    else
    {
        ...
        int job_jid = curr_job->jid;
        printf("[%d] (%d) %s", job_jid, pid, cmdline);
    }
    sigprocmask(SIG_SETMASK, &prev_one, NULL);
    if (!is_bg)
    {
...
int builtin_cmd(char **argv)
{
    ...
    else if (strcmp(cmd_name, "bg") == 0 || strcmp(cmd_name, "fg") == 0)
    {
        do_bgfg(argv);
        return 1;
    }
...
void do_bgfg(char **argv)
{
    ...
    if (strcmp(cmd, "bg") == 0)
    {
        ...
        if (argv[1][0] == '%')
        {
            ...
            job_jid = num;
            curr_job = getjobjid(jobs, job_jid);
            if (curr_job == NULL)
            {
                printf("%%%d: No such job\n", job_jid);
                return;
            }
            job_pid = curr_job->pid;
        }
        else
        {
            num = atoi(argv[1]);
            ...
            job_pid = num;
            curr_job = getjobpid(jobs, job_pid);
            if (curr_job == NULL)
            {
 ...
 * 注：参数pid没有使用，使用了全局`volatile sig_atomic_t fg_pid`
 */
void waitfg(pid_t pid)
{
    sigset_t mask, prev;
